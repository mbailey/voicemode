...
def should_repeat(text: str) -> bool:
    ...
    most PHRASE_MAX_LEADING_WORDS words, so long messages ending in a trigger
    word are not discarded (VM-291).
    """
    phrase = _matching_trailing_phrase(text, REPEAT_PHRASES, PHRASE_MAX_LEADING_WORDS)
...
def should_wait(text: str) -> bool:
    ...
    PHRASE_MAX_LEADING_WORDS words, so long messages ending in a trigger word
    are not discarded (VM-291).
    """
    phrase = _matching_trailing_phrase(text, WAIT_PHRASES, PHRASE_MAX_LEADING_WORDS)
    if phrase is not None:
