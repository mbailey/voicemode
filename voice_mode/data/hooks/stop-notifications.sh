...
# VoiceMode Connect: Stop hook for agent wake-up
# Blocks agent from stopping when there are pending inbox messages.
#
# IMPORTANT: Must check stop_hook_active to prevent infinite loops.
# When stop_hook_active=true, a previous stop hook already blocked —
# allow the stop to prevent the agent from being stuck.
...
# Exit early if Connect is not enabled (allow stop)
...
# CRITICAL: If stop_hook_active is true, a previous stop hook already blocked.
# We MUST allow the stop to prevent infinite loops.
if [ "$STOP_HOOK_ACTIVE" = "true" ]; then
  echo "stop_hook_active=true, allowing stop to prevent loop" >> "$DEBUG_LOG"
  exit 0
fi

if [ -z "$SESSION_ID" ]; then
  echo "No session_id, allowing stop" >> "$DEBUG_LOG"
  exit 0
fi
...
if [ -z "$AGENT_NAME" ]; then
  echo "No agent name for session $SESSION_ID, allowing stop" >> "$DEBUG_LOG"
  exit 0
fi

# Sanitize agent name
if [[ ! "$AGENT_NAME" =~ ^[a-zA-Z0-9._-]+$ ]]; then
  echo "Invalid agent name '$AGENT_NAME', allowing stop" >> "$DEBUG_LOG"
  exit 0
fi

echo "=== stop-notifications.sh $(date) session=$SESSION_ID agent=$AGENT_NAME ===" >> "$DEBUG_LOG"
...
if [ ! -f "$INBOX_FILE" ]; then
  echo "No inbox file, allowing stop" >> "$DEBUG_LOG"
  exit 0
fi

# Get the last delivered message ID
LAST_DELIVERED_ID=""
...
while IFS= read -r line; do
  ...
  if [ -n "$TEXT" ]; then
    ...
    MESSAGES="${MESSAGES}From ${FROM}: ${TEXT}"
    LATEST_ID="$MSG_ID"
  fi
done < "$INBOX_FILE"

if [ -z "$MESSAGES" ]; then
  echo "No pending messages, allowing stop" >> "$DEBUG_LOG"
  exit 0
fi

echo "Blocking stop: pending messages found, latest_id=$LATEST_ID" >> "$DEBUG_LOG"

# Mark messages as delivered (the agent will see them in the block reason)
if [ -n "$LATEST_ID" ]; then
...
cat <<HOOKEOF
{"decision": "block", "reason": "You have pending VoiceMode Connect messages:\n${ESCAPED_MESSAGES}"}
HOOKEOF
