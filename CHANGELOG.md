...
- **`--wait` for service status — block until a voice service is ready (VM-523, [#220](https://github.com/mbailey/voicemode/pull/220))** — The unified `service` status action gains `wait` and `timeout` parameters (CLI and MCP) so scripts and agents can block until whisper/kokoro/mlx-audio/voicemode is actually accepting requests instead of writing their own poll loop. Ships `get_service_port` / `is_service_ready` / `wait_for_service` helpers with stderr progress dots.
