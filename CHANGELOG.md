...
- **`voicemode doctor` — a one-shot installation health check ([#300](https://github.com/mbailey/voicemode/pull/300))** — A `claude doctor`-style command that runs read-only checks across Installation, Platform, Dependencies, Audio, Services, Configuration, Connect, and Updates, printing pass/fail/warn/skip verdicts with the exact fix command for anything wrong. Supports `--format json` for scripting and honours `--no-color`/`NO_COLOR`. The older `voicemode diag` is deprecated in favour of it.
