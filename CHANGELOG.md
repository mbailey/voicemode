...
- **In-process Parakeet STT backend for fast, fully-local English speech-to-text on Apple Silicon** — Add `parakeet://local` to `VOICEMODE_STT_BASE_URLS` to transcribe with NVIDIA's Parakeet ASR (`mlx-community/parakeet-tdt-0.6b-v3`) running directly via MLX, with **no HTTP hop** — unlike whisper.cpp / mlx-audio / OpenAI, which are reached over an OpenAI-compatible endpoint. On an M-series Mac this measured **~3.7× faster than the whisper.cpp server** (≈0.28s vs ≈1.05s on an ~9s clip) at equal-or-better English accuracy. It slots into the existing STT failover chain as just another ordered endpoint, so the recommended config keeps whisper/OpenAI as fallback for the language tail (Parakeet v3 covers English + 25 European languages, not e.g. Thai): `VOICEMODE_STT_BASE_URLS=parakeet://local,http://127.0.0.1:2022/v1,https://api.openai.com/v1`. **Apple Silicon only, opt-in.** Requires the new optional extra (`uv tool install voice-mode[parakeet]`); the dependency is `arm64`-gated so Intel Macs / Linux / Windows are unaffected, and a missing dep cleanly falls through to the next endpoint. Override the model with `VOICEMODE_PARAKEET_MODEL`.
