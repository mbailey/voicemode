...
- **`voicemode uninstall` — a clear, no-think uninstall path (VM-1874, #497)** — There was no documented way to uninstall VoiceMode. A new top-level `voicemode uninstall [-y/--yes] [--remove-models] [--remove-all-data]` command cleanly reverses the installer in order: stops + removes the Whisper, Kokoro, mlx-audio, and voicemode ("serve") services; removes the Claude Code MCP registration (both `--scope user` and local); backs up (never deletes) `~/.voicemode/voicemode.env`; and removes the `voice-mode` package itself last, since it deletes the running binary. Voice clones, recordings, conversations, transcriptions, and models are **never** removed automatically, even with `--remove-all-data` — a residual-footprint report at the end says exactly what was removed and what's left behind. A Claude Code **plugin** install (`claude plugin install voicemode@voicemode`) is detected and reported separately, since `claude mcp remove` can't see a plugin-managed registration — the report tells you to run `claude plugin uninstall voicemode`. New `voicemode service uninstall whisper|kokoro|mlx-audio|voicemode` wrapper subcommands cover single-service removal too. See the new [README Uninstall section](README.md#uninstall) and the full [manual-removal reference](docs/reference/uninstall.md) for every path VoiceMode and its installer write to disk.

- **Native Windows support — VoiceMode runs without WSL (#232)** — Windows users no longer need WSL2 (whose audio path has known reliability problems — #341, #342, #98): the MCP server, the `converse` tool, and the CLI now run natively on Windows. Four Unix-only mechanisms were made portable: (1) the conch's `fcntl.flock` file locking moved behind a new `voice_mode/file_lock.py` (unchanged `flock` on Unix; `msvcrt.locking` on a high-offset byte on Windows, since Windows region locks are mandatory and would otherwise block payload reads — the SQLite trick); (2) five `os.kill(pid, 0)` liveness probes became `psutil.pid_exists()` — on Windows `os.kill` is `TerminateProcess` for any signal, so "checking" a lock holder's PID could kill an unrelated live process; (3) the silence-detection recording loop gained a dead-stream stall backstop (stops if no audio chunk arrives for a few seconds) so a stream that goes silent — device lost, backend delivering no callbacks — can no longer spin the loop forever, without ever truncating a healthy recording; (4) `VOICEMODE_DEBUG=true` no longer crashes converse on Windows (`resource.getrusage` → psutil). No behavior change on Linux/macOS — the conch suite passes 98/98 on both Windows 11 and Linux. Works with any OpenAI-compatible local STT/TTS endpoints (e.g. speaches + kokoro-fastapi in Docker).
...
- **`converse` `turns[]` grows an `ask` verb — run a multi-question voice survey in one call, get aligned replies back (VM-1775)** — Building on VM-1772's gap-free `turns[]` sequence, a turn can now be `{"ask": str, ...}` (or the back-compat alias `{"say": str, "wait_for_response": true}`) to **speak a question and collect the spoken reply**, so a whole multi-question survey — mixing plain `say` turns with `ask` turns — runs as **one call** instead of one `converse` round-trip per question. The reply-collecting pipeline keeps VM-1772's producer/consumer overlap (synthesizing the next turn while the current one is still being listened to) and applies the full control-channel matrix per turn: `stop` ends the survey with partials; `skip_forward` advances a `say` turn or barges an `ask` turn straight into listening ("answer early")/ends its capture early; `skip_back` and the spoken "repeat"/"wait" keywords replay-then-relisten, rescoped per question; and a standalone spoken **"break"** (`VOICEMODE_SURVEY_BREAK_PHRASES`) ends the survey early without recording the break utterance itself as a reply. Any `ask` turn switches the return value from VM-1772's plain summary string to a JSON survey result — `{"survey": {"completed", "asked", "answered", "stopped_at", "turns": [...]}}` — where **`turns` is always index-aligned to the input, length N**, so a partial run (stop/break/failure) still tells you exactly which questions were answered, which were skipped, and where it stopped. A per-reply is logged to the conversation log **the instant it's captured**, not batched at the end, so a killed/cancelled call never loses an already-given answer even though the tool's own return value is lost. On the CLI, repeatable **`--ask 'VOICE:TEXT'`** (parallel to `--say`) drives a survey directly, exiting **`0`** on full completion or **`3`** on a partial so scripts can branch without parsing JSON; `--script` accepts `ask` turns for free for mixed say/ask sequences. A live phase breadcrumb at `~/.voicemode/state.json` (VM-1793) reports the in-flight `speaking`/`listening` phase and turn/answered-count on every transition, doubling as a crash-recovery signal for a killed call. See `voicemode converse --help` and [docs/reference/control-channel.md](docs/reference/control-channel.md#controls-during-a-survey-turns-with-ask).
- **Survey "heard you" cue — an opt-in confirmation sound tells a captured answer from a missed one (VM-1859)** — During a pipelined multi-question `converse` survey, turns advance automatically without the assistant reacting to each answer, so it can be hard to tell whether a reply was actually captured or the question just timed out and moved on. A new opt-in **`ack`** switch plays a **short, content-free confirmation cue the moment an `ask` turn captures a reply** — and stays **silent on a timeout / no-speech** — so that audible contrast is the "heard you → advancing" vs "didn't hear → advancing" signal (field-requested while driving). The cue is deliberately distinct in pitch and contour from the listening-start and recording-finished chimes, and being content-free it needs **no per-answer synthesis**, so the survey's no-dead-air pipelining is preserved. **Default off** — existing surveys are unchanged. Opt in per turn (`{"ask": ..., "ack": true}`), survey-wide via the call-level `ack` argument, or on the CLI with **`voicemode converse --ack --ask ...`**. See `voicemode converse --help` and the `turns` schema.
- **Survey conventions — announce the question count and acknowledge answers at the next call (VM-1859)** — The `turns` schema and the voicemode skill now instruct the agent to **open a survey by announcing how many questions there are** (e.g. "I've got 3 quick questions") so the user knows a multi-turn survey is running, and to **acknowledge the answers just collected at the top of the next call** (content-aware acknowledgment can't be pipelined mid-survey). The "one question at a time" best practice now carves an explicit survey exception so the guidance no longer contradicts itself. Convention-only — no runtime change.
...
- **TTS failover across mixed providers now sends each provider the model it understands (VM-1390)** — A `VOICEMODE_TTS_BASE_URLS` chain that mixes providers needing **different** model identifiers — most commonly the Apple-Silicon chain `kokoro-fastapi → mlx-audio → openai` — used to break on failover: the single global `VOICEMODE_TTS_MODELS` name was sent to every endpoint, so any step that landed on a provider which didn't recognise that name failed (sending `tts-1` to mlx-audio makes it try to fetch a Hugging Face repo literally named `tts-1` → 404, and the request hangs). VoiceMode now **detects the provider it's about to call and resolves the model per endpoint**: kokoro-fastapi and OpenAI get `tts-1`, mlx-audio gets `mlx-community/Kokoro-82M-bf16`, so a default `converse` with kokoro-fastapi down fails over to mlx-audio and speaks without a manual `voice=`/model override. Resolution order is **explicit caller model → `VOICEMODE_TTS_MODELS_<PROVIDER>` → first compatible global `VOICEMODE_TTS_MODELS` entry → built-in per-provider default**; single-provider users and clone voices see no change. If you previously worked around this with a global `VOICEMODE_TTS_MODELS=mlx-community/Kokoro-82M-bf16`, you can delete it. See [docs/reference/environment.md](docs/reference/environment.md#per-provider-tts-model-resolution).
