...
- **Control channel — CD-style skip-back to replay the previous utterance (VM-1685)** — The control channel grows **CD transport** alongside the cassette-style pause/resume/stop: press **skip-back** to re-hear what the assistant just said. The first press restarts the current / most-recent utterance from its start; each further press steps to the one before, walking back through a small **history buffer** of recently rendered utterances — so on a walk you can ask "what was that bit again?" with a single button instead of asking the agent to repeat itself. It **composes with pause** (pause, then skip back to re-hear the bit from just before) and replays *cached audio only* — **no new agent turn, no STT, no model call** — so, like pause/resume/stop, it stays on the safe side and needs no special permission. Drive it with the new `voicemode control skip-back` verb, a Stream Deck button, a media key, or a raw `{"command":"skip_back"}` line. The history buffer captures every utterance **regardless of `SAVE_AUDIO`**, is bounded in memory (`VOICEMODE_HISTORY_BUFFER_SIZE`, default 8), and a new request/response `{"command":"status"}` query reports "now playing" + buffer position for status displays (programmatic / raw-socket; `query_status()` in Python). Existing pause/resume/stop behaviour and the control-socket security model are unchanged. Skip-*forward* (deterministic barge-in to the listen turn) is its sibling task VM-1739; once it also lands, the Stream Deck `deck.py` `SKIP_ENABLED` flag flips to light both buttons. See [docs/reference/control-channel.md](docs/reference/control-channel.md).
