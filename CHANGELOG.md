...
- **The voicemode plugin can be configured to connect to a streamable-HTTP server in addition to the default stdio (VM-1292)** — Set `VOICEMODE_MCP_URL` to point the plugin's bundled MCP at a remote (or same-machine) `voicemode serve` instead of spawning a local stdio process. Unset = the current local stdio default, byte-for-byte unchanged. Enables a plugin-only install (skills + commands + hooks, no local `voicemode`/Whisper/Kokoro) pointed at an audio host. Auth via secret-in-path (`/mcp/<secret>`) with optional `VOICEMODE_MCP_TOKEN` Bearer header — reuses the existing serve auth, no serve-side changes. The remote transport uses a distinct `voicemode-remote` MCP name (so it never degrades the working local server when the URL is unset); tools in remote mode are `mcp__voicemode-remote__*`. Documents the duplicate-MCP precedence rule (user/project `voicemode` wins over the plugin-bundled one) and the per-mode `disabledMcpjsonServers` recipes. See the [Claude Code Plugin guide](docs/guides/claude-code-plugin.md#transport-modes-local-stdio-vs-remote-http).
