...
- **Sony WH/WF-series headphones now get the full Bluetooth chime amplitude (VM-1883, #496)** — On macOS, CoreAudio reports many Bluetooth headphones by bare model name — Sony's flagship over-ear/earbud lines show up as just `WH-1000XM4` / `WF-1000XM5` — so `generate_chime()`'s device check (which matched only `airpod`/`bluetooth`/`bt`) misclassified them as built-in speakers and halved the chime amplitude (0.075 vs 0.15), making the listen chime hard to hear. The check now also matches the `wh-`/`wf-` model-number prefixes, so these play at the intended Bluetooth level. A name-based allowlist is a stopgap — querying the CoreAudio transport type would be the robust long-term fix (tracked as a follow-up). Verified on macOS 26 with WH-1000XM4. Thanks to @blakechasteen.
