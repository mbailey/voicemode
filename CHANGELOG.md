...
- **`conch give` can now summon *any* running session, not just one already in line (VM-1637)** — `voicemode conch give <session>` (and the MCP `conch(action="give", target=…)`) used to require the target to already be in the waiter queue. Now, when the named target isn't waiting, VoiceMode discovers it among the **running** sessions (via `session list`), auto-enqueues it as a `callback` waiter, makes it the next acquirer, and pings it to take the floor (VM-1625) — all in one command. This is the godmode operator's "you, speak now": in a two-party voice session you can hand the conch to the *other* agent without it having had to request the floor first. Behaviour for an existing waiter is unchanged (a waiting target is granted exactly as before, with no discovery call); an *ambiguous* token is reported rather than guessed; giving to the current holder is a clear no-op (it already has the floor); and if session discovery is unavailable the command degrades to the prior "not waiting" message. The CLI and MCP share one implementation, so the two front ends can never diverge.
- **Notify-on-give — an idle agent granted the conch gets pinged instead of missing its turn (VM-1625)** — When the floor is handed to an agent that *isn't* actively waiting (a `callback`-mode waiter, or one you `voicemode conch give`/`bump` the conch to), VoiceMode now actively pushes a "your turn" nudge — `🐚 You've been granted the conch — call converse() to take the floor.` — into the grantee's tmux pane via the session control surface, instead of relying on it to notice. This is the *push* half of the conch's "pull for watchers, push for the idle": an agent already polling in a `wait` call still pulls the floor itself (no nudge, no double-delivery), while an idle grantee is told to come and take it. Local agents are nudged by session id (falling back to a project match); a remote grantee instead finds the grant waiting on its next `converse()`/`conch status` (an MCP channel notification seam is left for VM-970). The push is best-effort — a missing session tool or tmux never disturbs `give`/`bump`. When the floor is handed on automatically as a holder releases, the nudge is delivered off the release path, so it never delays the next speaker.
- **`voicemode conch` — observe and manage the multi-agent voice lock from the CLI (VM-1616)** — A new command group over the conch (VoiceMode's single-speaker lock) and the ordered waiter queue (VM-1613): **`voicemode conch status`** shows who holds the floor and who's waiting, in order (human table or `--json`); **`give <session>`** hands the floor to a named waiting session (matched by session-id or agent name) so it acquires next; **`bump`** drops the current holder and promotes the next in line (the bumped session must re-request); **`release`** (alias `clear`) force-clears a stale or stuck lock; and **`wait`** blocks until it's your turn, bounded by `--timeout`. It's a pure front end over the on-disk conch state — the single source of truth — so it stays consistent with `converse`'s own coordination. First of two equal front ends; an MCP equivalent for remote agents follows (VM-1622).
- **Remote agents can drive the conch queue over MCP — full parity with the CLI (VM-1622)** — A new MCP **`conch`** tool gives agents on a **streamable-HTTP** voicemode server (which can't reach the host's `~/.voicemode/` conch files) the same queue control the CLI has: one composite tool with an `action` arg mirroring the CLI verbs — **`status`** (holder + ordered queue), **`callback`**, **`wait`**, **`heartbeat`**, **`leave`**, plus the operator overrides **`give`** / **`bump`** / **`release`**. It's the second of *two equal front ends*: the CLI (VM-1616) and this tool now share one implementation (`voice_mode/conch_ops.py`), so a `give`/`bump`/`release` issued over MCP mutates the **same** on-disk state the CLI does — the two can never diverge. **Default delivery is callback** (register-and-return): a blocking await-turn can exceed a streamable-HTTP client's request timeout, so joining a busy conch returns your queue position immediately and your turn is delivered out-of-band; `wait` is still offered but **hard-capped** (`VOICEMODE_CONCH_MCP_WAIT_CAP`, default 25 s) well under typical client timeouts. A remote agent has no host PID, so its liveness is a heartbeat **TTL** (`VOICEMODE_CONCH_REMOTE_TTL`, default 90 s) the tool refreshes on every `wait`/`callback`/`heartbeat` call — a waiter past its TTL is auto-pruned so a dead remote agent never wedges the queue (`session_id` is the required, stable queue/grant key, since there's no `CLAUDE_CODE_SESSION_ID` over HTTP). Remote *push* notify-on-give lands with VM-970; until then the grant is discovered on the agent's next `status`/`heartbeat`/`callback` call. The tool is opt-in via `VOICEMODE_TOOLS_ENABLED`/`_DISABLED` (the default set stays `converse`,`service`). Completes the CLI⇄MCP parity pair under epic VM-1610.
- **`converse` joins the waiter queue when the conch is busy — wait or callback, your choice (VM-1619, folds in VM-1415)** — When another agent holds the voice channel, `converse` is now a first-class participant in the ordered waiter queue (VM-1613) instead of a silent poll-and-block. **`wait_for_conch` stays the opt-in gate** — left at its default (`false`) a busy `converse` returns *immediately* with a status that names the holder and tells you that you're not queued (you're never silently blocked). Set it `true` (or to a number of seconds) and you join the queue, where the new **`conch_mode`** option decides how you're served: **`wait`** blocks until the floor is granted to you, and **`callback`** registers you and returns *right away* with your queue position so your agent can do other work — your turn is delivered when granted (the out-of-band push lands in VM-1625; for now, watch `voicemode conch status` or run `voicemode conch wait`). The default mode is configurable via **`VOICEMODE_CONCH_MODE`** (default `wait`), absorbing VM-1415. Two wins over the old behaviour: **visibility** — a waiting `converse` now shows up in `voicemode conch status` with its mode and position instead of polling where no one can see it — and **fairness** — the floor is handed out in FIFO order via the queue's grant hint, so multiple waiters no longer thunder in and race for it (and `conch give` / `conch bump` remain the deliberate line-cutting overrides). Existing `wait_for_conch` callers keep working unchanged.
...
- **Conch holdership — keep the floor across turns in a multi-agent voice session (VM-1433, folds in VM-1562)** — A new `hold_conch` option on `converse` lets an agent retain exclusive voice access *between* turns, so a second agent can no longer slip in at a turn boundary while the first is thinking. It's **opt-in and intentional**: default behaviour is unchanged (the conch is fully released after each exchange); set `hold_conch=true` when you're asking a question you'll answer or otherwise intend to keep speaking. The hold auto-extends every turn and is released by your next `converse(hold_conch=false)`, by your process dying, or by a short idle-expiry safety valve (`VOICEMODE_CONCH_HOLD_EXPIRY`, default 10s — a refreshed between-turns TTL; see VM-1649 below). A companion **`pause_conversation(seconds, message)`** tool holds the floor across a deliberate pause (e.g. while doing background work) and resumes without losing your turn. The conch lock payload now also records the holder's **`session_id`** (caller-provided, with a stdio-only `VOICEMODE_SESSION_ID` / `CLAUDE_CODE_SESSION_ID` env fallback), **`project_path`**, and the **`voice`** in use, so tooling can show *which session, on which project, in which voice* holds the channel. The voice field (VM-914) lets a second agent read the holder's voice and pick a different one to avoid a voice clash — VoiceMode just exposes it; your instructions decide what to do with it. A queued caller (`wait_for_conch`) now waits through a live hold instead of bouncing after 60s — the default wait rose to 300s and accepts a per-call number of seconds — and still fast-fails the instant the holder's process dies. Implemented on a **single lock file** (the hold is the existing conch left in place and marked, not a second file), avoiding the races a two-file design invites. Design and original local patch contributed by **Tony Zhang** ([@mcteamster](https://github.com/mcteamster), [PR #463](https://github.com/mbailey/voicemode/pull/463)); reimplemented on the single-file model with his collaboration.

### Fixed

- **An idle conch *hold* no longer wedges the floor for everyone — short, refreshed TTL (VM-1649)** — `hold_conch=true` keeps the floor *between* turns, but its idle-expiry safety valve defaulted to **300s**, so an agent that stopped speaking could sit on the channel for ~5 minutes while every other agent waited (and any agent that conversed even once per 5-minute window kept the floor indefinitely). The hold is now a **short, converse-refreshed TTL**: the default drops to **10s** (`VOICEMODE_CONCH_HOLD_EXPIRY`, tune toward 5s), each `converse(hold_conch=true)` re-stamps it to `now + TTL`, and stopping lets it lapse within the window so the queue promotes the next waiter — no more 30-minute wedge. A new per-call **`conch_hold_timeout`** (seconds) on `converse` overrides the default for *this* hold; crucially the chosen TTL is now stamped as an absolute `expires` into the conch lock so **other** agents honour it cross-process (previously the override only lived in the holder's own process and was a no-op for everyone else, which is what let stale holds linger). `pause_conversation` re-stamps well within the shortened window, so a maintained pause still never lapses. `converse(hold_conch=false)` releases immediately, unchanged.
- **A callback waiter at the head of the queue no longer starves the agents blocking behind it (VM-1625)** — When an idle `callback`-mode waiter sat at the head of the conch queue, releasing the floor promoted it as the next acquirer — but a callback waiter never takes the floor itself (it waits to be pinged), so its standing grant blocked *every* actively-waiting agent behind it until they timed out. The queue now promotes the first blocking (`wait`-mode) waiter, skipping any leading callback waiters and pinging each to come back, so a blocking agent acquires straight away rather than waiting out an idle one ahead of it. A lone callback waiter (with no one blocking behind it) is unchanged.
