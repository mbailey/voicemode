...
- **Block timeline measurement for silence profiles (VM-1773)** — `SilenceProfile` now carries `first_speech_start` and `recording_end` fields, and exposes a `blocks()` method returning an ordered sequence of `(kind, start, end)` tuples where `kind` is `"speech"` or `"gap"`. This forms the foundation for downstream block-level measurement and reporting (VAD/STT timing visibility, silence pattern classification).
- **Explicit turn handoff: `silence_release_sec` scalar lets the assistant keep the floor through hesitation (VM-1787)** — A new `silence_release_sec` parameter (default 0) lets the assistant pause mid-thought without handing you the mic: `0` = current VAD behavior (immediate release on silence), `N` = tolerate N seconds then release, `-1` = never release on silence alone. `listen_duration_max` default raised from 120s to 180s, hard-capped at 300s. `disable_silence_detection` becomes a deprecated alias for `-1`. Handles the class of "thinking pauses" that classical VAD cuts off too early.
- **Silence observability: turn-level silence profile + transcript markers (VM-1787)** — Every `converse` turn reports a silence profile in the `Silence:` result field (total silence, max gap, pre-speech duration, and marker list) so you can respond to hesitation. Pauses ≥2s appear as inline `⟨pause Ns⟩` / `⟨pre-speech Ns⟩` transcript markers, letting agents "read" the assistant's hesitation and adjust tone, pacing, or content accordingly.
